...
int HGCalDDDConstants::maxCellsInRow(int lay, int row, int subSec, bool reco) const {
  ...
  return maxCellsInRow(h, bl, tl, alpha, index.second,row);
}
...
int HGCalDDDConstants::maxCellsInRow(float h, float bl, float tl, float alpha, 
				float cellSize, int row) const {
...
std::pair<int,int> HGCalDDDConstants::newCell(int cell, int layer, int sector, 
  ...
  /*if (layer==1) {
    std::cout << "---------------------------------------------------------" << std::endl
	      << "-- initial sector : " << sector << "/" << subsector << " kx " << kx << std::endl
	      << " -- row " << ky << " maxcells " << maxCellsInRow(layer,ky,subsector,true) << std::endl; 
	      }*/
  if (ky < 0 || ky > maxRows(layer, true)) {
    //no valid neighbour ! Return invalid values
    cell = maxCells(true);
    return std::pair<int,int>(cell,sector*subsector);
  }
  if (kx < 0) {
    //just change subsector 
    subsector =-subsector;
    //decrement kx => 5 4 3 2 1 0 0 1 2 3 4 5
    kx = -kx - 1;
  }
  if (kx >= maxCellsInRow(layer,ky,subsector,true)) {
    //change sector
    sector   += -1.*subsector;
    subsector =-subsector;
    if (sector < 1)             sector = nSectors;
    else if (sector > nSectors) sector = 1;
    int overflow = kx + 1 - maxCellsInRow(layer,ky,subsector,true);
    kx = maxCellsInRow(layer,ky,subsector,true)-overflow;
  }
  /*if (layer==1) {
    std::cout << "-- final sector : " << sector << "/" << subsector << " kx " << kx << std::endl
	      << " -- row " << ky << " maxcells " << maxCellsInRow(layer,ky,subsector,true) << std::endl
	      << "---------------------------------------------------------" << std::endl; 
	      }*/
