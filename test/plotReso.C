...
int plotReso() {
  ...
  std::string outfile = "InfoPlots_0_trueunconv_SR1";
  ...
  std::string cut1pca = "converted1==0 && eTrue1/cosh(etaTrue1)>40 && TMath::Abs(etaTrue1)>1.6 && TMath::Abs(etaTrue1)<2.8 && (TMath::Nint(TMath::Abs(phiTrue1)/(2*TMath::Pi())*360.)%20<9 || TMath::Nint(TMath::Abs(phiTrue1)/(2*TMath::Pi())*360.)%20>11) && invalidDetidPCA1==0 && invalidNeighbourPCA1==0 && dRTrue1 < 0.05";//calibratedE(eSR3Reco1,etaPCA1)/eSC1>0.9
  std::string cut2pca = "converted2==0 && eTrue2/cosh(etaTrue2)>40 && TMath::Abs(etaTrue2)>1.6 && TMath::Abs(etaTrue2)<2.8 && (TMath::Nint(TMath::Abs(phiTrue2)/(2*TMath::Pi())*360.)%20<9 || TMath::Nint(TMath::Abs(phiTrue2)/(2*TMath::Pi())*360.)%20>11) && invalidDetidPCA2==0 && invalidNeighbourPCA2==0 && dRTrue2 < 0.05";//calibratedE(eSR3Reco2,etaPCA2)/eSC2>0.9
  ...
  std::string var[nV] = {
    ...
    "dRTrue",
    "eSR1RecoOvereTrue","eSR5RecoOvereTrue",
    ...
    "eSR3RecoOvereTrue",
    "xPCA","yPCA","zPCA","eSR3RecoOvereTrue","eSR1RecoPCAOvereTrue","eSR1RecoPCAOvereTrue",
  ...
  //t->Draw("eSR1Reco1:eTrue1>>h2",cut1.c_str());
  //t->Draw("eSR1Reco2:eTrue2>>+h2",cut2.c_str());
  TH2F *h2 = new TH2F("h2",";egen (GeV);ereco 3x3 (mips); photons",60,0,600,60,0,600);
  t->Draw("calibratedE(eSR1Reco1,etaTrue1):eTrue1>>h2",cut1.c_str());
  t->Draw("calibratedE(eSR1Reco2,etaTrue2):eTrue2>>+h2",cut2.c_str());
  myc[0]->Clear();
  myc[0]->Divide(1,2);
  ...
  cor.str("");
  //cor << "(eSR1Reco1-" << offset3 << ")/(" << slope3 << "*eTrue1):TMath::Abs(etaTrue1)>>h2eta";
  //cor << "(calibratedE(eSR1Reco1,etaTrue1)-" << offset3 << ")/(" << slope3 << "*eTrue1):TMath::Abs(etaTrue1)>>h2eta";
   cor << "calibratedE(eSR1Reco1,etaTrue1)/eTrue1:etaTrue1>>h2eta";
  t->Draw(cor.str().c_str(),cut1.c_str());
  cor.str("");
  //cor << "(eSR1Reco2-" << offset3 << ")/(" << slope3 << "*eTrue2):TMath::Abs(etaTrue2)>>+h2eta";
  //cor << "(calibratedE(eSR1Reco2,etaTrue2)-" << offset3 << ")/(" << slope3 << "*eTrue2):TMath::Abs(etaTrue2)>>+h2eta";
  cor << "calibratedE(eSR1Reco2,etaTrue2)/eTrue2:etaTrue2>>+h2eta";
  ...
  t->Draw("calibratedE(eSR1RecoPCA1,etaPCA1):eTrue1>>h2pca",cut1pca.c_str());
  t->Draw("calibratedE(eSR1RecoPCA2,etaPCA2):eTrue2>>+h2pca",cut2pca.c_str());
  //TH2F *h2pca = new TH2F("h2pca",";egen (GeV);ereco 3x3 (mips); photons",60,0,600,1000,0,50000);
  //t->Draw("eSR1RecoPCA1:eTrue1>>h2pca",cut1.c_str());
  //t->Draw("eSR1RecoPCA2:eTrue2>>+h2pca",cut2.c_str());
  h2pca->Draw("colz");
  ...
  cor.str("");
  //cor << "(eSR1RecoPCA1-" << offset3pca << ")/(" << slope3pca << "*eTrue1):TMath::Abs(etaTrue1)>>h2pcaeta";
  cor << "calibratedE(eSR1RecoPCA1,etaPCA1)/eTrue1:etaTrue1>>h2pcaeta";
  t->Draw(cor.str().c_str(),cut1pca.c_str());
  cor.str("");
  //cor << "(eSR1RecoPCA2-" << offset3pca << ")/(" << slope3pca << "*eTrue2):TMath::Abs(etaTrue2)>>+h2pcaeta";
  cor << "calibratedE(eSR1RecoPCA2,etaPCA2)/eTrue2:etaTrue2>>+h2pcaeta";
  ...
  t->Draw("eSR1Reco1:eTrue1>>h25",cut1.c_str());
  t->Draw("eSR1Reco2:eTrue2>>+h25",cut2.c_str());
  h25->Draw("colz");
  ...
  cor.str("");
  cor << "(eSR1Reco1-" << offset5 << ")/(" << slope5 << "*eTrue1):TMath::Abs(etaTrue1)>>h25eta";
  t->Draw(cor.str().c_str(),cut1.c_str());
  cor.str("");
  cor << "(eSR1Reco2-" << offset5 << ")/(" << slope5 << "*eTrue2):TMath::Abs(etaTrue2)>>+h25eta";
  t->Draw(cor.str().c_str(),cut2.c_str());
  //  t->Draw("eSR1Reco1/eTrue1:TMath::Abs(etaTrue1)>>h25eta",cut1.c_str());
  //t->Draw("eSR1Reco2/eTrue2:TMath::Abs(etaTrue2)>>htmp5eta",cut2.c_str());
  h25eta->Draw("colz");
  ...
  */
  ...
  //t->Draw("eSR1Reco1/eTrue1:phiTrue1>>h2phi",cut1.c_str());
  //t->Draw("eSR1Reco2/eTrue2:phiTrue2>>+h2phi",cut2.c_str());
  cor.str("");
  //cor << "((eSR1Reco1-" << offset3 << ")/(" << slope3 << "*eTrue1)-" << offset3eta << ")/" << slope3eta << ":phiTrue1>>h2phi";
  //cor << "(calibratedE(eSR1Reco1,etaTrue1)-" << offset3 << ")/(" << slope3 << "*eTrue1):phiTrue1>>h2phi";
  cor << "calibratedE(eSR1Reco1,etaTrue1)/eTrue1:phiTrue1>>h2phi";
  t->Draw(cor.str().c_str(),cut1.c_str());
  cor.str("");
  //cor << "((eSR1Reco2-" << offset3 << ")/(" << slope3 << "*eTrue2)-" << offset3eta << ")/" << slope3eta << ":phiTrue2>>+h2phi";
  //cor << "(calibratedE(eSR1Reco2,etaTrue2)-" << offset3 << ")/(" << slope3 << "*eTrue2):phiTrue2>>+h2phi";
  cor << "calibratedE(eSR1Reco2,etaTrue2)/eTrue2:phiTrue2>>+h2phi";
  t->Draw(cor.str().c_str(),cut2.c_str());
  ...
  cor.str("");
  cor << "calibratedE(eSR1RecoPCA1,etaPCA1)/eTrue1:phiTrue1>>h2phipca";
  t->Draw(cor.str().c_str(),cut1pca.c_str());
  cor.str("");
  cor << "calibratedE(eSR1RecoPCA2,etaPCA2)/eTrue2:phiTrue2>>+h2phipca";
  t->Draw(cor.str().c_str(),cut2pca.c_str());
  ...
  cor.str("");
  //cor << "(eSR5RecoPCA1-" << offset3pca << ")/(" << slope3pca << "*eTrue1)";
  cor << "calibratedE(eSR5Reco1,etaTrue1)/eTrue1";
  var1[5] = cor.str();
  
  cor.str("");
  //cor << "((eSR1Reco1-" << offset3 << ")/(" << slope3 << "*eTrue1)-" << offset3eta << ")/" << slope3eta;
  //cor << "(calibratedE(eSR1Reco1,etaTrue1)-" << offset3 << ")/(" << slope3 << "*eTrue1)";
  cor << "calibratedE(eSR1Reco1,etaTrue1)/eTrue1";
  var1[4] = cor.str();
  
  cor.str("");
  //cor << "((eSR1Reco1-" << offset3 << ")/(" << slope3 << "*eTrue1)-" << offset3eta << ")/" << slope3eta;
  //cor << "(calibratedE(eSR1Reco1,etaTrue1)-" << offset3 << ")/(" << slope3 << "*eTrue1)";
  cor << "calibratedE(eSR1RecoPCA1,etaPCA1)/eTrue1";
  var1[23] = cor.str();
  
  cor.str("");
  //cor << "(eSR5RecoPCA2-" << offset3pca << ")/(" << slope3pca << "*eTrue2)";
  cor << "calibratedE(eSR5Reco2,etaTrue2)/eTrue2";
  var2[5] = cor.str();

  cor.str("");
  //cor << "((eSR1Reco2-" << offset3 << ")/(" << slope3 << "*eTrue2)-" << offset3eta << ")/" << slope3eta;
  //cor << "(calibratedE(eSR1Reco2,etaTrue2)-" << offset3 << ")/(" << slope3 << "*eTrue2)";
  cor << "calibratedE(eSR1Reco2,etaTrue2)/eTrue2";
  var2[4] = cor.str();

  cor.str("");
  //cor << "((eSR1Reco2-" << offset3 << ")/(" << slope3 << "*eTrue2)-" << offset3eta << ")/" << slope3eta;
  //cor << "(calibratedE(eSR1Reco2,etaTrue2)-" << offset3 << ")/(" << slope3 << "*eTrue2)";
  cor << "calibratedE(eSR1RecoPCA2,etaPCA2)/eTrue2";
  var2[23] = cor.str();
  ...
  //cor.str("");
  //cor << "(eSR3Reco2-" << offset5 << ")/(" << slope5 << "*eTrue2)";
  //var2[5] = cor.str();
  ...
  for (unsigned iV(0); iV<nV; ++iV){//loop on variables
    ...
    if (var2[iV].find("PCA")!=var2[iV].npos) t->Draw(cor.str().c_str(),cut2pca.c_str());
    ...
    //gStyle->SetOptFit(1111);
    if (iV==4 || iV==5 || iV==23){
      //hist[iV]->Fit("gaus","LR","same",0.95,1.05);
      hist[iV]->GetXaxis()->SetRangeUser(0.6,1.2);
      hist[iV]->GetXaxis()->SetTitle("#frac{e_{3#times3}}{e_{true}}");
      lat.SetTextSize(0.035);
      double minX = std::max(hist[iV]->GetBinLowEdge(1),0.6);
      lat.DrawLatex(minX,1.08*hist[iV]->GetMaximum(),"CMSSW_6_2_0_SLHC25_patch2 Relval Hgg PU140");

      lat.SetTextSize(0.05);
      sprintf(buf,"n_{#gamma}=%3.0f",hist[iV]->GetEntries());
      lat.DrawLatex(minX+0.05,0.95*hist[iV]->GetMaximum(),buf);

      double err = 0;
      double sigeff = effSigmaMacro(hist[iV],err);
      sprintf(buf,"#sigma_{eff}=%3.2f #pm %3.2f %%",sigeff*100,err*100);
      lat.DrawLatex(minX+0.05,0.85*hist[iV]->GetMaximum(),buf);
